...
use super::{super::Error, PACKET_LENGTH_LEN};
...
impl super::Cipher for AesSshCipher {
	fn key_len(&self) -> usize { self.0().key_length() }

	fn nonce_len(&self) -> usize { self.0().iv_length() }
...
impl super::SealingKey for SealingKey {
	fn padding_length(&self, payload:&[u8]) -> usize {
		// note: the .blocksize() method reports 1 for CTR, which is not what we
		// need...
		let block_size = 16;
