...
impl From<String> for CryptoVec {
	fn from(e:String) -> Self { CryptoVec::from(e.into_bytes()) }
}
...
impl IndexMut<RangeFull> for CryptoVec {
	fn index_mut(&mut self, _:RangeFull) -> &mut [u8] { self.deref_mut() }
}
...
impl IndexMut<RangeTo<usize>> for CryptoVec {
	fn index_mut(&mut self, index:RangeTo<usize>) -> &mut [u8] { self.deref_mut().index_mut(index) }
}
...
#[cfg(not(windows))]
unsafe fn mlock(ptr:*const u8, len:usize) { libc::mlock(ptr as *const c_void, len as size_t); }
#[cfg(not(windows))]
unsafe fn munlock(ptr:*const u8, len:usize) { libc::munlock(ptr as *const c_void, len as size_t); }
...
#[cfg(windows)]
unsafe fn mlock(ptr:*const u8, len:usize) { VirtualLock(ptr as LPVOID, len as SIZE_T); }
#[cfg(windows)]
unsafe fn munlock(ptr:*const u8, len:usize) { VirtualUnlock(ptr as LPVOID, len as SIZE_T); }
...
impl CryptoVec {
	...
	/// assert!(v.is_empty())
	/// ```
	pub fn clear(&mut self) { self.resize(0); }
